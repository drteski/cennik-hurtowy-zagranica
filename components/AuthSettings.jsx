...
const AuthSettings = () => {
  ...
  const authorize = async (e) => {
    e.preventDefault();
    await axios.post("/api/auth/settings", { pass }).then((data) => {
      setError(!data.data.authorized);
      cookieCutter.set("authorizedSettings", `${data.data.authorized}`);
      router.refresh();
    });
  };
    ...
    <div className="h-[100dvh] w-[100dvw] grid grid-cols-1 grid-rows-[116px_auto] min-w-[768px]">
      ...
      <div className="flex justify-center items-center">
        <form onSubmit={authorize}>
          <h1 className="text-7xl text-center font-bold mb-16">Settings</h1>
          {error ? (
